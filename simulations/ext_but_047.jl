...
function fed_batch_dFBA(N0,V0,nFE,S,t_max,t_min,V_max,c_G,glu,atp,oxy,xxx,pro,aon,dil,vlb,vub,d)
    ...
    phi1 = 1e+1 # factor on outer objective
    ...
    @NLconstraints(m, begin
        ...
        NLc_t_switch,  sum(tFE[i] for i in 1:22) == 16.65
        NLc_ctrl1[i=1:22],   q[xxx,i] - vub[xxx] == 0
        NLc_ctrl2[i=23:nFE], q[xxx,i] - vlb[xxx] <= .001
        V[end,end] == V_max
    ...
    for i in 1:nFE
        set_lower_bound(rtFE[i],0.5)
        set_upper_bound(rtFE[i],2)
        set_lower_bound(Fx[i],0.)
        set_upper_bound(Fx[i],10.)
        for mc in 1:nA
